...
(function () {
  ...
  function process(t) {
    ...
    if (!rec || n > rec.n) {
      ...
      /* Mode ajuste (defaut) : on reduit toujours les colonnes pour tenir dans le
         conteneur, plutot que de laisser un defilement horizontal. Le mode etendu
         (window.__TFB_FIT = false) conserve les largeurs naturelles. */
      var ajuste = (window.__TFB_FIT !== false) && !!avail;
      var fit = ajuste ? true : !(avail && natW > avail + 2);
      var w = nat, i;
      if (ajuste) { w = trimToFit(nat.slice(), avail); }
      else if (!fit) { w = []; for (i = 0; i < nat.length; i++) w.push(nat[i] + PAD); }
  ...
  /* Recalcul complet a la demande (changement de mode ajuste / etendu). */
  window.__tfbRelock = function () {
